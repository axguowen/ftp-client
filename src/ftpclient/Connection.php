...
namespace axguowen\ftpclient;

use axguowen\FtpClient;
use axguowen\ftpclient\utils\DirTool;
...
class Connection
{
     ...
     * @return $this
     */
    public function setClient(FtpClient $client)
    {
        // 设置客户端
        $this->client = $client;
        // 返回自身
    ...
    {
        // 获取临时目录
        $tempDir = $this->client->getConfig('temp_dir');
        // 如果为空
        if(empty($tempDir)){
            // 目录分隔符
